...
Finally, there is some evidence that individuals respond differently to different treatments. One large study that examined multiple medication-assisted quit attempts found that individuals who switched medications were more likely to quit than individuals who used the same medication again or who did not use a medication on the first quit attempt but added one at the second [@heckmanEffectivenessSwitchingSmokingCessation2017]. Relatedly, there is some evidence that re-treatment with the same medication as a previous, unsuccessful quit attempt is not effective [@fioreClinicalPracticeGuideline2008; @tonnesenRecyclingNicotinePatches1993] Indeed, Gonzales and colleagues found that “abstinence rates are more than threefold lower for NRTs and twofold lower for bupropion” during re-treatment compared to initial treatment using the same medication [@gonzalesRetreatmentVareniclineSmoking2014, p. 391]. Despite a shared pharmacological mechanism of action, a pilot trial showed that treatment adherence improves when individuals are given the opportunity to sample various NRT medications pre-quit, suggesting differential preferences [@cropseyPilotTrialVivo2017]. Additionally, clinical research related to other psychological and psychiatric disorders has demonstrated differential treatment benefit on an individual basis (e.g., antipsychotic medications for schizophrenia [@roussidisReasonsClinicalOutcomes2013], psychosocial interventions for depression [@cohenTreatmentSelectionDepression2018]), suggesting it is worth investigating whether the same is true in smoking cessation.

#### Previous precision mental health & machine learning research

Within cigarette smoking cessation, work has been done!

Reasonable body of evidence predicting smoking cessation (overall) - i.e., predicting who will or won't succeed
- this is useful! to help understand mechanisms that underlie smoking cessation success, may allow us to intervene in mechanisms before a quit attempt, design preventative interventions, etc.
- but does not allow us to select the "right" treatment for an individual

Some PMH work in cig smoking using biomarkers and genetic features, some using neuroimaging
- nicotinic receptor gene
- NMR (biomarker)
- these studies DO show some evidence of differential responding (although there is some mixed evidence)
- however, come with down sides re: accessibility/availability of screening among others (see chen review), not well suited for implementation
- also Insel 2015 re: "there isn't a biomarker for these dx" & commentary on addiction vs oncology stuff

A lot of research cites the promise of their work leading to personalized/precision treatment, but there is not an actionable way forward to true treatment selection. 

<!--
emphasize - certainly a lot of work has been done, but...
- suboptimal targets (not tx selection, more often predicting who will succeed or not, or whether an individual tx will work or not)
...
main effect of tx_match

tx_match X time interaction
