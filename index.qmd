...
The goal of this project was to evaluate the clinical benefit of a treatment selection model. Specifically, we assessed whether using our model to select among medications for smoking cessation can increase abstinence following quitting smoking. The treatment selection model was developed using data from a previously completed comparative effectiveness trial of varenicline, C-NRT, and nicotine patch [@bakerEffectsNicotinePatch2016]. 
...
^keep data section as is above, move treatment success outcome paragraphs to relevant subsections

1. Model building (but NOT selection/evaluation?)

2. Evaluating clinical benefit
- we identified the best model configuration (i.e., combination of model characteristics)
- we got predictions for each person, held-out (explain LOOCV briefly)
- process for evaluating/analysis plan
- is clinical benefit equal across groups?

3. Evaluating model performance (is there signal to increase confidence in benefit?)
- nested CV process
- auROC
- satisficing metric
- is performance equal across groups? (maybe cut)
