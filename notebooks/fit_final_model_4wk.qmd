...
```{r}
dim(feat_all)
# N = 1086 (not doing upsampling)
# p = 1173

# model fit with new_data = NULL
feat_all_null <- rec_prepped_full |> 
  bake(new_data = NULL) 
# N = 1430, p = 1173

model_null <- fit_best_model(best_model = best_config, 
                             feat = feat_all_null, 
                             ml_mode = "classification")
...
tidy(model_null) |> 
  ...
  nrow()
# 181 features retained (similar to N = 1085 models)
...
tidy(model_null) |> 
  ...
  slice(1:10) |> 
  print_kbl(digits = 4)
# parameter estimates are similar to those from N = 1085 models

# fit a model with identical config except no resampling
config_none <- best_config |> 
  mutate(resample = "none")

rec_none <- build_recipe(d = d, config = config_none)

rec_prepped_none <- rec_none |> 
  prep(training = d, strings_as_factors = FALSE)

feat_none <- rec_prepped_none |> 
  bake(new_data = NULL)
# N = 1086, p = 1173

model_none <- fit_best_model(best_model = config_none,
                             feat = feat_none,
                             ml_mode = "classification")
...
tidy(model_none) |> 
  ...
  filter(abs(estimate) > 0) |> 
  nrow()
# 133 features retained (similar to mistake new_data = d model)
...
tidy(model_none) |> 
  ...
  slice(1:10) |> 
  print_kbl(digits = 4)
# parameter estimates are similar to mistake model
