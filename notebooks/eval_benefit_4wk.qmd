...
```{r}
fixef_tx <- summary(model_bayes)$coefficients["c_tx_match", "Estimate"]
fixef_wk <- summary(model_bayes)$coefficients["c4_week_log_2", "Estimate"]
fixef_int <- summary(model_bayes)$coefficients["c_tx_match:c4_week_log_2", "Estimate"]
intercept <- summary(model_bayes)$coefficients["(Intercept)", "Estimate"]

d_fig_alt <- d |> 
  select(c_tx_match, c4_week_log_2) |> 
  distinct(c_tx_match, c4_week_log_2) |> 
  mutate(int = c_tx_match * c4_week_log_2) |> 
  mutate(log_odds = (fixef_tx * c_tx_match) + (fixef_wk * c4_week_log_2) +
           (fixef_int * int) + intercept) |> 
  ...
  mutate(probabilities = odds / (1 + odds))
```

Log odds figure
```{r}
d_fig_alt |> 
  ggplot(aes(x = week_cat, y = log_odds, fill = tx_match)) +
  geom_col(position = "dodge") +
  labs(
    x = "Week On-Study",
    y = "Log Odds",
...
```{r}
d_fig_alt |> 
  ggplot(aes(x = week_cat, y = probabilities, fill = tx_match)) +
  geom_col(position = "dodge") +
  labs(
    x = "Week On-Study",
