...
Cigarette smoking could benefit greatly from combining precision mental health and machine learning. Smoking remains an enormous public health burden. Fourteen percent of U.S. adults smoke cigarettes daily, and smoking is the leading cause of preventable death, accounting for 480,000 deaths annually10–12. 

<!-- 
ADD TO THIS: shouldn't be LONG but should be expanded 
consider comparing to other substances, specific health consequences, citing work about impact on longevity of extra years smoking
many people don't get treatment!! numbers for this & success rates without tx
-->

#### Treatment effectiveness for smoking cessation

Despite the severity of the problem <!-- possibly: For those who do get treatment-->, the best available smoking cessation treatments are only modestly effective. The medications varenicline and combination nicotine replacement therapy (C-NRT) are consistently identified as the most effective options when combined with psychosocial counseling (Cahill et al., 2013), and guidelines recommend that clinicians consider either of these two medications first given their established efficacy (Fiore et al., 2008). These medications appear to be equally (though modestly) effective: a meta-analysis demonstrated comparable effectiveness rates (Cahill et al., 2013), and the first randomized controlled trial (RCT) directly comparing varenicline and C-NRT did not find a difference between them (Baker et al., 2016). 
...
#### Previous precision mental health research
...
- using predictors that are not well-suited for implementation
...
<!-- ADD
feature engineering i did incl ordinal vs dummy scoring, yeo johnson transformations, constructing tx interactions, scaling (content depends on glmnet only or multiple algorithms)

if glmnet only:
- ordinal vs dummy scoring of ordered categorical variables (eg likert scale items on self-report measures)
- dummy coding of unordered categorical variables
- yeojohnson transformation of numeric variables
- interactions between treatment dummy codes & all other variables
...
We trained and evaluated two separate classification models: one model predicting 4-week abstinence, and one model predicting 26-week (6 month) abstinence. <!-- either:

We considered three well-established statistical algorithms (XGBoost, Random Forest, and Elastic Net Logistic Regression [GLMNet]) that vary across characteristics expected to affect model performance (e.g., flexibility, complexity, handling higher-order interactions natively) (CITE KUHN & ISL). Candidate model configurations differed across sensible values of associated hyperparameters (XGBoost: mtry, tree depth, learning rate; Random Forest: mtry, others?; GLMNet: alpha, lambda) using grid search. All other hyperparameters were set to tidymodels package defaults.

OR

All model configurations used the statistical algorithm Elastic Net Logistic Regression (GLMNet). This algorithm was chosen because it aligns with our primary goal of building a treatment selection in several ways. First, it allows for explicit inclusion of interaction terms (i.e., including interactions between treatment and all other variables). Second, GLMNet performs a degree of dimensionality reduction because it penalizes model complexity; thus, a model using this algorithm may require assessing fewer features than are initially considered in the model. Candidate model configurations differed across sensible values of the hyperparameters alpha and lambda. -->

Candidate model configurations also differed on outcome resampling method. We considered no resampling, up-sampling, down-sampling, and the synthetic minority oversampling technique (SMOTE <!-- ; GLMNet and XGBoost only-->). These resampling techniques were used to create majority/abstinence to minority/smoking ratios in the held-in training data ranging from X:1 to X:1. 

We also considered several feature sets and feature engineering steps across model configurations. Feature sets could include either items (i.e., individual items within a self-report measure) or scales (i.e., total scale and subscale scores derived from items in a self-report measure). All other features (e.g., demographic variables) were included across model configurations. Feature engineering steps also differed across model configurations regarding how ordinal data were scored. Specifically, ordinal data could be considered as numeric (e.g., 1 - 7) or dummy coded (e.g., 7 dummy code features for a 7-level variable). Numeric and unordered nominal data were treated identically across configurations.

##### Performance metric

<!-- cite paper(s) about roc -->
...
<!-- cite paper in match folder about problems with selecting & evaluating in same sample -->

We used nested cross-validation for model training, selection, and evaluation with auROC. Cross-validation allows for rigorous consideration of many model configurations (i.e., combinations of feature sets, statistical algorithms, resampling techniques, and hyperparameters) and prioritizes performance in new data not used for model training.
...
Following model evaluation, we completed another round of 1 repeat of 10-fold cross-validation using the full dataset. A single best model configuration was selected using median auROC across the 10 held-out folds; importantly, model performance is used *only* for selection and not evaluation in this phase. We fit our final model using this best model configuration in the full dataset, which was then used for clinical benefit analyses (below).
...
main effect of best vs other treatment

tx X time interaction
- follow-up simple effects if needed

supplemental aim 2 analyses: effects coding? 
