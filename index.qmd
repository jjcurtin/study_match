...
Following model evaluation, we completed another round of 1 repeat of 10-fold cross-validation using the full dataset. A single best model configuration was selected using median auROC across the 10 held-out folds. Importantly, model performance in this phase is used *only* for selection and not evaluation [@Krstajic et al., 2014]. This best selected model configuration was then used for clinical benefit analyses (below).
